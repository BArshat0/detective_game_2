...
class MysteryAudioEngine {
  ...
  /** Play celebratory royal/mystery arpeggio for achievement unlocked */
  public playHonorUnlockedSound() {
    this.initCtx();
    if (!this.ctx) return;
    const now = this.ctx.currentTime;
  ...
  /** Play forensic evidence inspect / scan click */
  public playEvidenceInspectSound() {
    this.initCtx();
    if (!this.ctx) return;
    const now = this.ctx.currentTime;
    const osc = this.ctx.createOscillator();
