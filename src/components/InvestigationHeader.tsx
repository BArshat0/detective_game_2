...
import { ArrowLeft, Brain, Calendar, Compass, FileText, MessageSquare, ShieldCheck, Target, CheckCircle } from 'lucide-react';
...
export default function InvestigationHeader({
  ...
  return (
    <section className="relative overflow-hidden rounded-[28px] border border-white/10 glass-panel bg-slate-900/80 p-5 sm:p-7 shadow-2xl text-white">
      <div className="relative z-10 flex flex-col gap-5">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-start gap-3">
            <button
              onClick={onBack}
              className="mt-1 rounded-full border border-white/10 bg-white/5 p-2 text-slate-300 transition-colors hover:bg-white/10 hover:text-white focus:outline-none cursor-pointer"
            ...
            <div>
              ...
              <p className="mt-1 text-xs font-mono text-slate-400">Case file #{caseData.id.toUpperCase().replace('CASE_', '')} · Every detail is a lead.</p>
            </div>
          </div>

          <div className="flex shrink-0 items-center gap-2">
            <div className="rounded-2xl border border-[#ff8533]/30 bg-[#ff8533]/10 px-3.5 py-2 text-right">
              <div className="text-[8px] font-mono uppercase tracking-wider text-slate-400">Detective rank</div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 px-3.5 py-2 text-center">
              <div className="text-[8px] font-mono uppercase tracking-wider text-slate-400">Field XP</div>
        ...
        <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-center">
          <div className="rounded-2xl border border-[#ffb829]/20 bg-[#ffb829]/5 p-4">
            <div className="mb-1 flex items-center gap-2 text-[10px] font-mono font-black uppercase tracking-wider text-[#ffb829]">
              <Target className="h-3.5 w-3.5 text-[#ff8533]" /> Current Objective
            </div>
            <p className="max-w-3xl text-xs leading-relaxed text-slate-200 font-mono font-medium">{objective}</p>
          </div>
          <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-xs font-mono text-slate-300">
            <CheckCircle className="h-4 w-4 text-[#ff8533]" />
            ...
            return (
              <button
                ...
                className={`group rounded-2xl border px-3 py-3 text-left transition-all focus:outline-none cursor-pointer ${
                  selected 
                    ? 'border-[#ff8533] bg-[#ff8533]/20 text-white shadow-lg shadow-[#ff8533]/10' 
                    : 'border-white/10 bg-white/5 text-slate-300 hover:border-white/20 hover:bg-white/10'
                }`}
                aria-current={selected ? 'page' : undefined}
              >
                <div className="flex items-center gap-2">
                  <Icon className={`h-4 w-4 shrink-0 ${selected ? 'text-[#ff8533]' : 'text-slate-400 group-hover:text-white'}`} />
                  <span className={`text-xs font-bold truncate ${selected ? 'text-white' : 'text-slate-200 group-hover:text-white'}`}>{label}</span>
                </div>
                <div className={`mt-1 pl-6 text-[9px] font-mono uppercase tracking-wider ${selected ? 'text-[#ff8533]' : 'text-slate-500'}`}>{hint}</div>
              </button>
            );
          })}
        </nav>
